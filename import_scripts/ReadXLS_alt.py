...
def importExcelFile(filename, column=False, column2=False, top=None):
    ...
    for s in wb.sheets():
        ...
        for row in range(s.nrows):
            count+=1
            values = []
            if column!=False and column2!=False: ### Return results from a single designated column number
                if top!=None:
                    val1 = str(s.cell(row,column-1).value)
                    try: val2 = str(s.cell(row,column2-1).value)
                    except: val2 = ''
                    if 'ene' in val1:
                        continue
                    if val2=='': val2 = prior_val2
                    else:
                        prior_val2 = val2
                        count=0
                    if count>top:
                        continue
                #if count < 10: print val1,val2
                #else: sys.exit()
                try: values.append([val1,val2])
                except Exception: pass
            elif column!=False: ### Return results from a single designated column number
                if top!=None:
                    if count>top:
                        continue
...
def processWorksheetMarkers(worksheet_data):
    ...
    for worksheet in worksheet_data:
        for value in worksheet_data[worksheet]:
            compiled_results.append([worksheet,value])
    return compiled_results

def exportCompiledResults(output,compiled_results):
    export_object = open(output,'w')
    count=0
    for (worksheet,value) in compiled_results:
        if isinstance(value, list):
            out = string.join([worksheet]+value[0],'\t')
            export_object.write(out+'\n')
        else:
            try: export_object.write(worksheet+'\t'+value[0]+'\n')
            except: pass
        count+=1
    export_object.close()
...
if __name__ == '__main__':
    ...
    if len(sys.argv[1:])<=1:  ### Indicates that there are insufficient number of command-line arguments
        ...
        options, remainder = getopt.getopt(sys.argv[1:],'', ['xls=','i=','column=','column2=','output=','o=','top='])
    ...
    worksheet_data = importExcelFile(xls,column=column,column2=column2,top=top)
