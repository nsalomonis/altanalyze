...
class Main(wx.Frame):
    def __init__(self,parent,id):
        ...
        self.species=''
        #PANELS & WIDGETS              
        self.panel = wx.Panel(self, id=2, pos=(200,0), size=(600,45), style=wx.RAISED_BORDER)  ### NS changed from 900     
    ...
    def CellGraph(self, event):
        ...
        if Wikipathway_Flag == 1:
            ...
            for i in range(len(VarGridSet)):
                ...
                if(len(y) > 0):
                    VariableBox.append(i)
        q_barrel = []
        self.control.write(str(VariableBox) + "\n")
        ...
        self.control.write(str(q_barrel) + "\n")
        Values_List = []
        for box in q_barrel:
            output_box = []
            for value in box:
        ...
        self.control.write(str(Values_List) + "\n")
        Output_Values_List = []
        ...
        rects1 = plt.bar(index, Output_Values_List, bar_width,
                        alpha=opacity,
                        color='b',
                        label='')
        ...
        for i in VariableBox:
            ...
            for p in range(500):
                if(q < 0):
                    break
                q = q - 1
                try: self.control.write(str(int(VarGridSet[q][1])) + "\n")
                except: break    
                q_box.append(q)
            q_barrel.append((q_box))
        self.control.write(str(q_barrel) + "\n")
    ...
    def InteractiveTabChoose(self, event):
        self.control.write(str(self.nb.GetSelection()) + "\n") 
    ...
    def SelectedTopTreeID(self, event):
        ...
        if(TXT_FLAG == 1):
            ...
            try:
                ...
                AVERAGE_FIND_FLAG = re.findall("AVERAGE", self.DirFileTxt)
                ...
                for line in table_file:
                    ...
                    table_file_contents.append((line))
                    if len(DATASET_FIND_FLAG) > 0:# or len(AVERAGE_FIND_FLAG) >0:
                        if rowCount == InitialMaxImport: break
                    rowCount+=1
                    #if MaxImport > rowCount: break
    ...
    def InteractivePanelUpdate(self, event):
        ...
        if(len(Heatmap_RegEx) > 0):
            ...
            options_open = open("options.txt", "rU")
            heatmap_array = []
            self.heatmap_ids = {}
            self.heatmap_translation = {}
