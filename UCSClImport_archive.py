...
def importEnsExonStructureData(species):
    ...
    if round == 2: filename = 'AltDatabase/ucsc/'+species+'/'+species+'_UCSC_NonGene_structure_mrna.txt'
...
def importUCSCExonStructureData(species):
    ...
    ###Simple function to import and organize exon/transcript data
    if analysis_type == 'mRNA': filename = 'AltDatabase/ucsc/'+species+'/all_mrna.txt'; input_gene_file = filename; merged_ac_count=0;ac_count=0
...
def filterBuiltAssociations(species):
    if analysis_type == 'mRNA': input_gene_file = 'AltDatabase/ucsc/'+species+'/all_mrna.txt'
...
def runUCSCEnsemblAssociations(Species,Analysis_type,Round,run_from_scratch):
    global species; species = Species; global round; round = Round
...
def updateUCSCEnsemblAssociations(Species,Analysis_type,run_from_scratch):
    Round = 1; runUCSCEnsemblAssociations(Species,Analysis_type,Round,run_from_scratch)
    if Analysis_type != 'mRNA':
        Round = 2; runUCSCEnsemblAssociations(Species,Analysis_type,Round,run_from_scratch)
